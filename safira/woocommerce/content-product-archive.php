...
$safira_shopclass = Safira_Class::safira_shop_class('');
$colwidth = 3;
if($safira_shopclass=='shop-fullwidth') {
	if(isset($safira_opt)){
		$woocommerce_loop['columns'] = $safira_opt['product_per_row_fw'];
	...
	if(isset($safira_opt)){
		$woocommerce_loop['columns'] = $safira_opt['product_per_row'];
		if($woocommerce_loop['columns'] > 0){
			$colwidth = round(12/$woocommerce_loop['columns']);
		}
	}
				...
				<div class="product-rating">
					...
					<?php if ( comments_open() ) : ?>
						<div class="product-review">
							<?php printf( 
								_n( '%s review', '%s reviews', $review_count, 'safira' ),
								'<span class="count">' . 
								esc_html( $review_count ) . 
								'</span>' ); 
							?>
							<a href="#reviews" class="woocommerce-review-link" rel="nofollow">
								<?php esc_html_e('Add Your Reviews', 'safira');?>
							</a>
						</div>
					<?php endif ?>
				</div>
